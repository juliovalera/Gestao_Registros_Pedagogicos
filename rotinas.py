...
class RotinasDocentesWindow(tk.Toplevel):
    ...
    def _build(self) -> None:
        ...
        self.space_combo = ttk.Combobox(filters, state="readonly", width=26)
        ...
        self.context_combo = ttk.Combobox(filters, state="readonly", width=28, values=[""] + CONTEXTOS_ATUACAO)
...
class RotinaDocenteForm(tk.Toplevel):
    ...
    def _build(self) -> None:
        ...
        ttk.Label(frame, text="Turma ou público").grid(row=7, column=0, sticky="w", pady=4)
        self.publico_entry = ttk.Entry(frame, width=62)
        self.publico_entry.grid(row=7, column=1, sticky="ew", pady=4)

        ttk.Label(frame, text="Título *").grid(row=8, column=0, sticky="w", pady=4)
        ...
        self.descricao_text = self._add_text_field(frame, 10, "Descrição da atividade *", 6)
        ...
        self.observacoes_text = self._add_text_field(frame, 14, "Observações", 4)
    ...
    def _load_references(self) -> None:
        ...
        self.professor_listbox.delete(0, tk.END)
        self.space_map = {"": None}
        for record in refs["professores"]:
