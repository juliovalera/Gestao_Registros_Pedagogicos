...
class RotinasDocentesWindow(tk.Toplevel):
    ...
    def _build(self) -> None:
        ...
        ttk.Button(professor_actions, text="Limpar sele??o", command=self._clear_professor_selection).pack(side="left")
        ...
        ttk.Label(frame, text="Espa?o").grid(row=6, column=0, sticky="w", pady=4)
        self.space_combo = ttk.Combobox(frame, state="readonly", width=58)
        self.space_combo.grid(row=6, column=1, sticky="ew", pady=4)

        ttk.Label(frame, text="Turma ou p?blico").grid(row=7, column=0, sticky="w", pady=4)
        self.publico_entry = ttk.Entry(frame, width=62)
        self.publico_entry.grid(row=7, column=1, sticky="ew", pady=4)

        ttk.Label(frame, text="T?tulo *").grid(row=8, column=0, sticky="w", pady=4)
        self.titulo_entry = ttk.Entry(frame, width=62)
        self.titulo_entry.grid(row=8, column=1, sticky="ew", pady=4)

        ttk.Label(frame, text="Tags").grid(row=9, column=0, sticky="w", pady=4)
        self.tags_entry = ttk.Entry(frame, width=62)
        self.tags_entry.grid(row=9, column=1, sticky="ew", pady=4)

        self.descricao_text = self._add_text_field(frame, 10, "Descri??o da atividade *", 6)
        self.objetivos_text = self._add_text_field(frame, 11, "Objetivos", 4)
        self.recursos_text = self._add_text_field(frame, 12, "Recursos utilizados", 4)
        self.encaminhamentos_text = self._add_text_field(frame, 13, "Encaminhamentos", 4)
        self.observacoes_text = self._add_text_field(frame, 14, "Observa??es", 4)
        self.evidence_input = EvidenceInput(frame, height=4)
        self.evidence_input.grid(row=15, column=0, columnspan=2, sticky="ew", pady=(8, 0))
    ...
    def save(self) -> None:
        ...
        if not data["data"] or not data["professor_ids"] or not data["categoria"] or not data["titulo"] or not data["descricao_atividade"]:
            show_error(
                "Campos obrigat?rios",
                "Preencha data, ao menos um professor, categoria, t?tulo e descri??o da atividade.",
                self,
            )
            return
