...
from models import OPCOES_TRIPLAS, TIPOS_AUSENCIA
from utils import (
    DateInput,
    TimeInput,
    center_window,
...
class AusenciasWindow(tk.Toplevel):
    ...
    def _build(self) -> None:
        ...
        self.tipo_combo = ttk.Combobox(filters, values=[""] + TIPOS_AUSENCIA, state="readonly", width=26)
        ...
        self.keyword_entry = ttk.Entry(filters, width=44)
        ...
        ttk.Button(filters, text="Limpar filtros", command=self.clear_filters).grid(row=2, column=5, padx=4, pady=4)
    ...
    def clear_filters(self) -> None:
        ...
        for combo in (self.professor_combo, self.space_combo, self.tipo_combo):
            combo.set("")
        self.search()
...
class AusenciaForm(tk.Toplevel):
    ...
    def _build(self) -> None:
        ...
        ttk.Label(frame, text="Turma ou grupo afetado").grid(row=6, column=0, sticky="w", pady=4)
        self.turma_entry = ttk.Entry(frame, width=62)
        self.turma_entry.grid(row=6, column=1, sticky="ew", pady=4)

        ttk.Label(frame, text="Tipo de ausência *").grid(row=7, column=0, sticky="w", pady=4)
        self.tipo_combo = ttk.Combobox(frame, values=TIPOS_AUSENCIA, state="readonly", width=58)
        self.tipo_combo.grid(row=7, column=1, sticky="ew", pady=4)

        ttk.Label(frame, text="Havia comunicação prévia").grid(row=8, column=0, sticky="w", pady=4)
        self.comunicacao_combo = ttk.Combobox(frame, values=[""] + OPCOES_TRIPLAS, state="readonly", width=58)
        self.comunicacao_combo.grid(row=8, column=1, sticky="ew", pady=4)

        ttk.Label(frame, text="Houve substituição").grid(row=9, column=0, sticky="w", pady=4)
        self.substituicao_combo = ttk.Combobox(frame, values=[""] + OPCOES_TRIPLAS, state="readonly", width=58)
        self.substituicao_combo.grid(row=9, column=1, sticky="ew", pady=4)

        self.impacto_text = self._add_text_field(frame, 10, "Impacto observado", 5)
        self.providencia_text = self._add_text_field(frame, 11, "Providência tomada", 5)
