...
from models import ESPACO_TODOS, NIVEIS_GRAVIDADE, PROFESSOR_TODOS
from utils import (
    DateInput,
    EvidenceInput,
    TimeInput,
...
class IntercorrenciasWindow(tk.Toplevel):
    ...
    def _build(self) -> None:
        ...
        ttk.Label(filters, text="Espaço").grid(row=1, column=3, padx=4, pady=4, sticky="w")
        self.space_combo = ttk.Combobox(filters, state="readonly", width=22)
        self.space_combo.grid(row=1, column=4, padx=4, pady=4, sticky="ew")

        ttk.Label(filters, text="Tipo").grid(row=1, column=5, padx=4, pady=4, sticky="w")
        self.type_combo = ttk.Combobox(filters, state="readonly", width=18)
        self.type_combo.grid(row=1, column=6, padx=4, pady=4, sticky="ew")
        ...
        self.tags_entry.grid(row=2, column=5, columnspan=2, padx=4, pady=4, sticky="ew")
...
class IntercorrenciaForm(tk.Toplevel):
    ...
    def _build(self) -> None:
        ...
        ttk.Label(frame, text="Pessoas relacionadas").grid(row=5, column=0, sticky="w", pady=4)
        self.pessoas_entry = ttk.Entry(frame, width=62)
        self.pessoas_entry.grid(row=5, column=1, sticky="ew", pady=4)

        ttk.Label(frame, text="Nível de gravidade").grid(row=6, column=0, sticky="w", pady=4)
        self.gravidade_combo = ttk.Combobox(frame, values=[""] + NIVEIS_GRAVIDADE, state="readonly", width=58)
        self.gravidade_combo.grid(row=6, column=1, sticky="ew", pady=4)

        ttk.Label(frame, text="Tags").grid(row=7, column=0, sticky="w", pady=4)
        self.tags_entry = ttk.Entry(frame, width=62)
        self.tags_entry.grid(row=7, column=1, sticky="ew", pady=4)

        self.description_text = self._add_text_field(frame, 8, "Descrição objetiva *", 6)
        self.providencias_text = self._add_text_field(frame, 9, "Providências adotadas", 5)

        ttk.Label(frame, text="Encaminhado para").grid(row=10, column=0, sticky="w", pady=4)
        self.encaminhado_entry = ttk.Entry(frame, width=62)
        self.encaminhado_entry.grid(row=10, column=1, sticky="ew", pady=4)

        self.observacoes_text = self._add_text_field(frame, 11, "Observações", 4)
        self.evidence_input = EvidenceInput(frame, height=4)
        self.evidence_input.grid(row=12, column=0, columnspan=2, sticky="ew", pady=(8, 0))

        button_frame = ttk.Frame(frame)
        button_frame.grid(row=13, column=0, columnspan=2, sticky="e", pady=(12, 0))
